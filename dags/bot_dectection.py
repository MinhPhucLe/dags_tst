...
dag = DAG(
    'simple_task_for_testing_bot_detection',
    default_args=default_args,
    description='simple_task_for_testing',
    catchup=False,
    schedule_interval="15 * * * *",  # Run the DAG daily
)

spark_submit_task = SparkSubmitOperator(
    application='hdfs://hadoop-hadoop-hdfs-nn:9000/test_spark/bot_detection.py',  # Path to the Java Spark application JAR
    task_id='spark_submit_add_bot_detection',
    conn_id='spark_default',  # Connection ID for Spark (preconfigured in Airflow)
    verbose=True,
    name='spark_submit_add_bot_detection',
    conf={
